...
use Horde_ActiveSync_Driver_MockConnector;
...
class TestBase extends TestCase
{
    ...
    {
        $cache = new Horde_ActiveSync_SyncCache(self::$state, 'dev123', 'mike', self::$logger);
    ...
    protected function _testCacheFolders()
    {
        ...
        // First Fixture
        $folder = new Horde_ActiveSync_Message_Folder((['logger' => $log, 'protocolversion' => Horde_ActiveSync::VERSION_TWELVEONE]));
    ...
    {
        $cache = new Horde_ActiveSync_SyncCache(self::$state, 'dev123', 'mike', self::$logger);
        $newcache = new Horde_ActiveSync_SyncCache(self::$state, 'dev123', 'mike', self::$logger);
    ...
    {
        $cache = new Horde_ActiveSync_SyncCache(self::$state, 'dev123', 'bob', self::$logger);
        $this->assertEquals([], $cache->getFolders());

        $cache = new Horde_ActiveSync_SyncCache(self::$state, 'dev456', 'mike', self::$logger);
        $this->assertEquals([], $cache->getFolders());
    }
    ...
    {
        $as = $this->getMockBuilder(Horde_ActiveSync::class)->disableOriginalConstructor()->getMock();
        $as->logger = Horde_ActiveSync::_wrapLogger(self::$logger);
        $as->state = self::$state;
        if ($addDevice) {
